...
<div class="flex flex-col gap-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h2 class="text-white font-normal text-xl uppercase tracking-wider">Edit Artist</h2>
            <p class="text-[#53a1b3] text-sm">Update artist details</p>
        </div>
    ...
    <div class="bg-[#213042] border border-[#53a1b3]/10 p-6 rounded-[3px] max-w-2xl">
        <form action="{{ route('admin.artists.update', $artist->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-4">
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" name="name" value="{{ old('name', $artist->name) }}" class="w-full bg-[#141e24] border border-[#53a1b3]/20 text-white px-4 py-2 text-sm focus:outline-none focus:border-[#e96c4c] transition" required>
                </div>
                    ...
                    <div class="relative">
                        <select name="country_id" class="w-full bg-[#141e24] border border-[#53a1b3]/20 text-white px-4 py-2 text-sm focus:outline-none focus:border-[#e96c4c] transition appearance-none">
                            ...
                            @foreach($countries as $country)
                                <option value="{{ $country->id }}" {{ old('country_id', $artist->country_id) == $country->id ? 'selected' : '' }}>{{ $country->name }}</option>
                            @endforeach
                        </select>
                ...
                <div class="space-y-2">
                    ...
                    <textarea name="bio" rows="4" class="w-full bg-[#141e24] border border-[#53a1b3]/20 text-white px-4 py-2 text-sm focus:outline-none focus:border-[#e96c4c] transition">{{ old('bio', $artist->bio) }}</textarea>
                </div>
                    ...
                    <div class="flex items-center gap-4">
                        <div class="w-20 h-20 bg-[#141e24] rounded-full flex items-center justify-center border border-[#53a1b3]/20 overflow-hidden" id="preview-container">
                            @if($artist->image_path)
                                <img src="{{ Storage::url($artist->image_path) }}" class="w-full h-full object-cover">
                            @else
                        ...
                        <div class="flex-1">
                            ...
                            onchange="document.getElementById('preview-container').innerHTML = '<img src=\'' + window.URL.createObjectURL(this.files[0]) + '\' class=\'w-full h-full object-cover\'>'">
                             <p class="text-[10px] text-[#53a1b3]/50 mt-1">Leave empty to keep current image.</p>
                        </div>
                    </div>
                </div>
            </div>
